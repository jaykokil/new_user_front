...
const API_URL = import.meta.env.VITE_API_URL || "https://backend-all-tgww.onrender.com/api";

const STABLE_SAMPLE_COUNT = 8;
const STABLE_TOLERANCE_GRAMS = 2;

export default function BarcodeScaleConnector() {
  const scannerInputRef = useRef(null);
  const barcodeTimerRef = useRef(null);
  const recentWeightsRef = useRef([]);
  ...
  const [weightStatus, setWeightStatus] = useState("Waiting for stable weight");
  ...
  const connectScale = async () => {
    try {
      ...
      setWeightStatus("Waiting for stable weight");
  ...
  const unlockWeightReading = () => {
    weightLockedRef.current = false;
    recentWeightsRef.current = [];
    setCurrentWeight("");
    setRemainingML(0);
    setWeightStatus("Waiting for stable weight");
    setTimeout(() => {
  ...
  const handleIncomingWeight = (rawWeight) => {
    const cleanWeight = Math.round(Number(rawWeight));
    ...
    const samples = [...recentWeightsRef.current, cleanWeight].slice(-STABLE_SAMPLE_COUNT);
    recentWeightsRef.current = samples;

    if (samples.length < STABLE_SAMPLE_COUNT) {
      setWeightStatus("Reading weight...");
      return;
    }
    ...
    if (max - min <= STABLE_TOLERANCE_GRAMS) {
      const stableWeight = Math.round(samples.reduce((sum, item) => sum + item, 0) / samples.length);

      weightLockedRef.current = true;
      setCurrentWeight(stableWeight);
      setWeightStatus("Stable weight locked");
    } else {
      setWeightStatus("Waiting for weight to stabilize");
    }
  };
    ...
    try {
      ...
      while (true) {
        ...
        if (matches && matches.length > 0) {
          const latestWeight = matches[matches.length - 1];
          handleIncomingWeight(latestWeight);
  ...
  const saveReading = async () => {
    ...
    if (!currentWeight) {
      alert("Stable weight is not locked yet. Please wait until the weight is stable.");
      return;
    }
    ...
    <section className="machine-card">
      <div className="top-row">
        <div>
          ...
          <p className="muted">
            Scan barcode, fetch bottle details, lock stable weight, and calculate remaining ML.
          </p>
        </div>
      ...
      <div className="reading-grid">
        ...
        <label>Live Weight G<input value={liveWeight} readOnly /></label>
        <label>Locked Stable Weight G<input value={currentWeight} readOnly /></label>
      ...
      <div className="button-row">
        ...
        <button className="secondary-btn" onClick={unlockWeightReading}>
          Read Weight Again
        </button>
      </div>
